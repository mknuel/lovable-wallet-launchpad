...
export const supplySepoliaETH = async (account, usdAmount) => {
	...
	if (totalCollateralETH === 0) {
		return {
			success: false,
			message:
				"Transaction confirmed but no collateral registered. Check transaction or retry.",
...
export const borrowETH = async (account, usdAmount) => {
	...
	const ethAmount = usdToEth(usdAmount);
	console.log(
		`🔄 BORROW: Converting $${usdAmount} USD to ${ethAmount} ETH for borrowing`
	);
...
export const repayETH = async (account, usdAmount) => {
	...
	const ethAmount = usdToEth(usdAmount);
	console.log(
		`🔄 REPAY: Converting $${usdAmount} USD to ${ethAmount} ETH for repayment`
	);
...
export const getUserAccountData = async (userAddress) => {
	...
	try {
		...
		console.log(
			"📊 GET_ACCOUNT_DATA: No Aave positions found for user, returning zero values"
		);
