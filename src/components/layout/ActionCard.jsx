...
export const ActionCard = ({
	...
	const baseClasses = `flex h-[108px] items-center relative rounded-2xl transition-all duration-200 ${
		disabled
	...
	// Dynamic classes for normal cards based on theme
	const normalClasses = `flex flex-1 min-w-0 border pl-4 pr-[82px] pt-14 pb-7 border-solid ${
		isDarkMode
			? `bg-[#2a2a2a] border-[#444444] ${
					disabled ? "" : "hover:border-[#555555]"
			  }`
			: `bg-white border-[#D9D4D4] ${disabled ? "" : "hover:border-gray-300"}`
	}`;

	// Dynamic text colors based on theme
	const normalTextClasses = `text-lg font-bold leading-6 absolute left-4 top-14 ${
		isDarkMode ? "text-[#E0E0E0]" : "text-[#595959]"
	}`;
	...
	return (
		<button
			...
			aria-label={`${title} action`}>
			<div className={normalTextClasses}>{title}</div>
