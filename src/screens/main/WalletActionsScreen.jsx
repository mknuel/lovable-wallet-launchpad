...
const WalletActionsScreen = () => {
  ...
  const statsData = walletData?.data ? [
    { id: 'tokens', value: walletData.data.token || '0', label: t('wallet.tokens') || 'Tokens' },
    { id: 'crypto', value: walletData.data.balance || '0', label: t('wallet.crypto') || 'Crypto' },
    { id: 'loans', value: '0', label: t('wallet.loans') || 'Loans' }
  ] : [
    { id: 'tokens', value: '234', label: t('wallet.tokens') || 'Tokens' },
    { id: 'crypto', value: '190', label: t('wallet.crypto') || 'Crypto' },
    { id: 'loans', value: '715', label: t('wallet.loans') || 'Loans' }
  ];

  const actionOptions = [
    {
      id: 'send',
      label: t('wallet.actions.send') || 'Send your tokens to another DAO member or invite someone by phone to receive them',
      action: () => navigate(PATH_SEND_TOKENS)
    },
    {
      id: 'exchange',
      label: t('wallet.actions.exchange') || 'Exchange your tokens to EURX (€ Euro) or other Cryptocurrency',
